...
FIGURE_3N_TripletvsENTaPD1 <- function(TCF7pos_Tim3neg_DEG) {
    # image: png
    ...
    label.col <- "Gene"
    sig.col <- c("C_T_ENT+N803+aPD1_vs_T_ENT+aPD1_pval")
    pCutoff  = 0.05
    lfc.col <- c("C_T_ENT+N803+aPD1_vs_T_ENT+aPD1_logFC")
    FCcutoff = 0.26303
    ...
    additional_labels <- "Ccr7,Lef1,Gzmb,Ly6a,Icos"
    labSize <- 6    

    #Title and Axis labels Parameters
    change_sig_name <- "p-value"
    change_lfc_name <- "log2FC"
    title <- "T_ENT+N803+aPD1 vs. T_ENT+aPD1"
    #subtitle <- ""
...
print("template_FIGURE_3N_TripletvsENTaPD1.R #########################################################################")
...
var_FIGURE_3N_TripletvsENTaPD1<-FIGURE_3N_TripletvsENTaPD1(var_TCF7pos_Tim3neg_DEG)
saveRDS(var_FIGURE_3N_TripletvsENTaPD1, paste0(rds_output,"/var_FIGURE_3N_TripletvsENTaPD1.rds"))
