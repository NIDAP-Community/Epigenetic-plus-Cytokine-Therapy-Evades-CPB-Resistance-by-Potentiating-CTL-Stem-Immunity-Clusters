...
FIGURE_3J <- function(Cd44posGzmbneg_DEG) {
    # image: png
    ...
    #Basic Parameters:
    df.orig <- Cd44posGzmbneg_DEG
    label.col <- "Gene"
    sig.col <- c("C_LN_ENT+N803+aPD1_vs_LN_PBS_pval")
    pCutoff  = 0.05
    lfc.col <- c("C_LN_ENT+N803+aPD1_vs_LN_PBS_logFC")
    FCcutoff = 0.26303
    ...
    additional_labels <- "Lgals1,Ctla2a,Ccl5,Plac8,Gzmm,Cd7,Ccr2,Cxcr3,Ccl4,Runx2,Ccr5,Cd6,Cd5,Xcl1,Il2rb,Myc,Stat1,Tox"
    labSize <- 8    

    #Title and Axis labels Parameters
    change_sig_name <- "p-value"
    change_lfc_name <- "log2FC"
    title <- "LN_ENT+N803+aPD1 vs. LN_PBS"
    #subtitle <- ""
...
# Processing input variable: var_Cd44posGzmbneg_DEG
var_Cd44posGzmbneg_DEG<-readRDS(paste0(rds_output,"/var_Cd44posGzmbneg_DEG.rds"))

if (!('Seurat' %in% class(var_Cd44posGzmbneg_DEG))) { if (!(class(var_Cd44posGzmbneg_DEG) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_Cd44posGzmbneg_DEG <- as.data.frame(var_Cd44posGzmbneg_DEG)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_FIGURE_3J<-FIGURE_3J(var_Cd44posGzmbneg_DEG)
saveRDS(var_FIGURE_3J, paste0(rds_output,"/var_FIGURE_3J.rds"))
