...
DLSO_Cd44posGzmbneg <- function(FSO_TCF7pos_Tim3neg,SampleNames_CellTypes) {  # produces 2 color channels and the overlay
    ...
    marker1 <- "Cd44"
    marker_1_threshold <- 0.045
    marker_1_type <- "SCT"
    marker2 <- "Gzmb"
    marker_2_threshold <- 0.020
    marker_2_type <- "SCT"
    
    #Filter Parameters:
    filter_data <- TRUE
    parameter_name <- "Cd44posGzmbneg"
...
print("template_DLSO_Cd44posGzmbneg.R #########################################################################")
...
var_DLSO_Cd44posGzmbneg<-DLSO_Cd44posGzmbneg(var_FSO_TCF7pos_Tim3neg,var_SampleNames_CellTypes)
saveRDS(var_DLSO_Cd44posGzmbneg, paste0(rds_output,"/var_DLSO_Cd44posGzmbneg.rds"))
