...
FIGURE_3N_TripletvsENTN803 <- function(TCF7pos_Tim3neg_DEG) {
    # image: png
    ...
    #Basic Parameters:
    df.orig <- TCF7pos_Tim3neg_DEG
    label.col <- "Gene"
    sig.col <- c("C_T_ENT+N803+aPD1_vs_T_ENT+N803_pval")
    pCutoff  = 0.05
    lfc.col <- c("C_T_ENT+N803+aPD1_vs_T_ENT+N803_logFC")
    FCcutoff = 0.26303
    ...
    additional_labels <- "Icos,Il7r,Pdcd1,Ctla4,Gzmk,Junb,Ccr7,Tigit,Stat3,Jak2,Stat5b,Cd28,Kit,Gzmb,Myc,Foxp1,Bcl2,Sell"
    labSize <- 6    

    #Title and Axis labels Parameters
    change_sig_name <- "p-value"
    change_lfc_name <- "log2FC"
    title <- "T_ENT+N803+aPD1 vs. T_ENT+N803"
    #subtitle <- ""
...
print("template_FIGURE_3N_TripletvsENTN803.R #########################################################################")
...
# Processing input variable: var_TCF7pos_Tim3neg_DEG
var_TCF7pos_Tim3neg_DEG<-readRDS(paste0(rds_output,"/var_TCF7pos_Tim3neg_DEG.rds"))

if (!('Seurat' %in% class(var_TCF7pos_Tim3neg_DEG))) { if (!(class(var_TCF7pos_Tim3neg_DEG) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_TCF7pos_Tim3neg_DEG <- as.data.frame(var_TCF7pos_Tim3neg_DEG)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_FIGURE_3N_TripletvsENTN803<-FIGURE_3N_TripletvsENTN803(var_TCF7pos_Tim3neg_DEG)
saveRDS(var_FIGURE_3N_TripletvsENTN803, paste0(rds_output,"/var_FIGURE_3N_TripletvsENTN803.rds"))
