...
ProcessedInputSO <- function(Dataset,My_MetadataToAdd) {
...
# Processing input variable: var_Dataset
var_Dataset<-readRDS(paste0(rds_output,"/var_Dataset.rds"))

if (!('Seurat' %in% class(var_Dataset))) { if (!(class(var_Dataset) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_Dataset <- as.data.frame(var_Dataset)}}
#############################
...
var_ProcessedInputSO<-ProcessedInputSO(var_Dataset,var_My_MetadataToAdd)
