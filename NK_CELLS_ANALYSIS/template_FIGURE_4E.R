...
FIGURE_4E <- function(ENTN803aPD1vsENTN803, GSEA_Filtered1, DEGMarkers_Il1bnegS100a9neg, MetadataTable_recluster_Il1bnegS100a9neg, msigdb_v6_2_with_orthologs) {
...
## PARAMETERS
    ...
    genescore_df <- ENTN803aPD1vsENTN803
    gex_df <- DEGMarkers_Il1bnegS100a9neg
    metadata_df <- MetadataTable_recluster_Il1bnegS100a9neg
    gsea_df <- GSEA_Filtered1
...
if (grepl("LE", which_plot)) {
    ...
    samples_to_include = setdiff(c("p_val_ENT+N803+aPD1_vs_ENT+N803"), geneid)
...
for ( i in 1:length(gsea_list) ) {
    ...
    plotES = gg.plotES(ranks=genescore_df, gsea=gsea, ntop=10, add_top=TRUE, cex_top=6, image_size='full', gset=gset, line_top=topline, ES_colo = 'ES sign')
    gsea_out <- plotES[[3]]
    ...
    if (which_plot == "ES+RNK+LE") {
        ...
        metadata = prep.metadata(meta=metadata_df, gsea=gsea, samples=colnames(gex_le), Sample="My_Sample_Names", Group="orig_ident", transformation=gex_transformation, dropREF=drop_ref, reference=reference_phenotype, own_palette=c(), label_palette='Accent', filterSamples=filter_samples)
...
png(filename="FIGURE_4E.png", width=(pageWidth+5)*300, height=(pageHeight+5)*300, units="px", pointsize=4, bg="white", res=300, type="cairo") 
print(preview)
...
# Processing input variable: var_ENTN803aPD1vsENTN803
var_ENTN803aPD1vsENTN803<-readRDS(paste0(rds_output,"/var_ENTN803aPD1vsENTN803.rds"))

if (!('Seurat' %in% class(var_ENTN803aPD1vsENTN803))) { if (!(class(var_ENTN803aPD1vsENTN803) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_ENTN803aPD1vsENTN803 <- as.data.frame(var_ENTN803aPD1vsENTN803)}}
#############################


# Processing input variable: var_GSEA_Filtered1
var_GSEA_Filtered1<-readRDS(paste0(rds_output,"/var_GSEA_Filtered1.rds"))

if (!('Seurat' %in% class(var_GSEA_Filtered1))) { if (!(class(var_GSEA_Filtered1) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_GSEA_Filtered1 <- as.data.frame(var_GSEA_Filtered1)}}
#############################


# Processing input variable: var_DEGMarkers_Il1bnegS100a9neg
var_DEGMarkers_Il1bnegS100a9neg<-readRDS(paste0(rds_output,"/var_DEGMarkers_Il1bnegS100a9neg.rds"))

if (!('Seurat' %in% class(var_DEGMarkers_Il1bnegS100a9neg))) { if (!(class(var_DEGMarkers_Il1bnegS100a9neg) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_DEGMarkers_Il1bnegS100a9neg <- as.data.frame(var_DEGMarkers_Il1bnegS100a9neg)}}
#############################


# Processing input variable: var_MetadataTable_recluster_Il1bnegS100a9neg
var_MetadataTable_recluster_Il1bnegS100a9neg<-readRDS(paste0(rds_output,"/var_MetadataTable_recluster_Il1bnegS100a9neg.rds"))

if (!('Seurat' %in% class(var_MetadataTable_recluster_Il1bnegS100a9neg))) { if (!(class(var_MetadataTable_recluster_Il1bnegS100a9neg) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_MetadataTable_recluster_Il1bnegS100a9neg <- as.data.frame(var_MetadataTable_recluster_Il1bnegS100a9neg)}}
#############################
...
var_FIGURE_4E<-FIGURE_4E(var_ENTN803aPD1vsENTN803,var_GSEA_Filtered1,var_DEGMarkers_Il1bnegS100a9neg,var_MetadataTable_recluster_Il1bnegS100a9neg,var_msigdb_v6_2_with_orthologs)
saveRDS(var_FIGURE_4E, paste0(rds_output,"/var_FIGURE_4E.rds"))
