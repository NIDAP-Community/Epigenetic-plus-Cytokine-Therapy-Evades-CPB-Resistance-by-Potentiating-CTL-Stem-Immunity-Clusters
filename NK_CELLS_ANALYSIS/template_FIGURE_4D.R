...
FIGURE_4D <- function(DEGMarkers_Il1bnegS100a9neg) {
    ...
    #Primary Inputs:
    deg_table <- DEGMarkers_Il1bnegS100a9neg
    ...
    minimum_number_of_DEG_genes <- 94
    
    #Visual Parameters:
    number_of_pathways_to_plot <- 12 
    plot_top_pathways_up <- FALSE
    pathways_to_use_up <- c("HALLMARK_INFLAMMATORY_RESPONSE","HALLMARK_IL2_STAT5_SIGNALING","HALLMARK_INTERFERON_GAMMA_RESPONSE","HALLMARK_ALLOGRAFT_REJECTION","HALLMARK_HYPOXIA","HALLMARK_TNFA_SIGNALING_VIA_NFKB")
    plot_top_pathways_down <- TRUE
    pathways_to_use_down <- c() 
    pathway_axis_label_font_size <- 20
    pathway_axis_label_max_length <- 20
...
var_FIGURE_4D<-FIGURE_4D(var_DEGMarkers_Il1bnegS100a9neg)
saveRDS(var_FIGURE_4D, paste0(rds_output,"/var_FIGURE_4D.rds"))
