...
SuppFIGURE_3B <- function(ENTN803aPD1_vs_PBS, GSEA_Preranked, DEGMarkers_CD8_GzmbposKi67neg, MetadataTable_CD8GzmbposKi67neg, msigdb_v6_2_with_orthologs) {
...
## PARAMETERS
    ...
    genescore_df <- ENTN803aPD1_vs_PBS
    gex_df <- DEGMarkers_CD8_GzmbposKi67neg
    metadata_df <- MetadataTable_CD8GzmbposKi67neg
...
if (grepl("LE", which_plot)) {
    ...
    samples_to_include = setdiff(c("p_val_adj_ENT+N803+aPD1_vs_PBS"), geneid)
...
for ( i in 1:length(gsea_list) ) {
    ...
    if (which_plot == "ES+RNK+LE") {
        ...
        metadata = prep.metadata(meta=metadata_df, gsea=gsea, samples=colnames(gex_le), Sample="My_Sample_Names", Group="SCT_snn_res_0_2", transformation=gex_transformation, dropREF=drop_ref, reference=reference_phenotype, own_palette=c(), label_palette='Accent', filterSamples=filter_samples)
...
png(filename="SuppFIGURE_3B.png", width=(pageWidth+5)*300, height=(pageHeight+5)*300, units="px", pointsize=4, bg="white", res=300, type="cairo") 
print(preview)
...
print("template_SuppFIGURE_3B.R #########################################################################")
...
# Processing input variable: var_ENTN803aPD1_vs_PBS
var_ENTN803aPD1_vs_PBS<-readRDS(paste0(rds_output,"/var_ENTN803aPD1_vs_PBS.rds"))

if (!('Seurat' %in% class(var_ENTN803aPD1_vs_PBS))) { if (!(class(var_ENTN803aPD1_vs_PBS) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_ENTN803aPD1_vs_PBS <- as.data.frame(var_ENTN803aPD1_vs_PBS)}}
#############################


# Processing input variable: var_GSEA_Preranked
var_GSEA_Preranked<-readRDS(paste0(rds_output,"/var_GSEA_Preranked.rds"))

if (!('Seurat' %in% class(var_GSEA_Preranked))) { if (!(class(var_GSEA_Preranked) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_GSEA_Preranked <- as.data.frame(var_GSEA_Preranked)}}
#############################
...
# Processing input variable: var_MetadataTable_CD8GzmbposKi67neg
var_MetadataTable_CD8GzmbposKi67neg<-readRDS(paste0(rds_output,"/var_MetadataTable_CD8GzmbposKi67neg.rds"))

if (!('Seurat' %in% class(var_MetadataTable_CD8GzmbposKi67neg))) { if (!(class(var_MetadataTable_CD8GzmbposKi67neg) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_MetadataTable_CD8GzmbposKi67neg <- as.data.frame(var_MetadataTable_CD8GzmbposKi67neg)}}
#############################
...
var_SuppFIGURE_3B<-SuppFIGURE_3B(var_ENTN803aPD1_vs_PBS,var_GSEA_Preranked,var_DEGMarkers_CD8_GzmbposKi67neg,var_MetadataTable_CD8GzmbposKi67neg,var_msigdb_v6_2_with_orthologs)
saveRDS(var_SuppFIGURE_3B, paste0(rds_output,"/var_SuppFIGURE_3B.rds"))
