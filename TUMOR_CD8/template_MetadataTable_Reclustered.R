...
MetadataTable_Reclustered <- function(ReclusteredSO_CD8Tcells) {
...
seurat_object <- ReclusteredSO_CD8Tcells
...
#image: png
    png(filename="MetadataTable_Reclustered.png", width=image_width, height=image_height,      units="px", pointsize=4, bg="white", res=image_resolution, type="cairo") 
...
print("template_MetadataTable_Reclustered.R #########################################################################")
...
# Processing input variable: var_ReclusteredSO_CD8Tcells
var_ReclusteredSO_CD8Tcells<-readRDS(paste0(rds_output,"/var_ReclusteredSO_CD8Tcells.rds"))

if (!('Seurat' %in% class(var_ReclusteredSO_CD8Tcells))) { if (!(class(var_ReclusteredSO_CD8Tcells) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_ReclusteredSO_CD8Tcells <- as.data.frame(var_ReclusteredSO_CD8Tcells)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_MetadataTable_Reclustered<-MetadataTable_Reclustered(var_ReclusteredSO_CD8Tcells)
saveRDS(var_MetadataTable_Reclustered, paste0(rds_output,"/var_MetadataTable_Reclustered.rds"))
