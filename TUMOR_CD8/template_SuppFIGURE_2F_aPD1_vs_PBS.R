...
SuppFIGURE_2F_aPD1_vs_PBS <- function(DEGMarkers_Reclustered_CD8CD3) {
    # image: png
    ...
    label.col <- "Gene"
    sig.col <- c("C_aPD1_vs_PBS_pval")
    pCutoff  = 0.05
    lfc.col <- c("C_aPD1_vs_PBS_logFC")
    FCcutoff = 0.26303
   
    
    #Label Parameters
    value_to_sort_the_output_dataset <- "p-value"
    no_genes_to_label <- 40
    use_only_addition_labels <- TRUE
    additional_labels <- "Prf1,Ccl3,Gzmb,Entpd1,Havcr2,Runx2,Cxcr6,Tox,Cd226,Irf8,Runx1,Runx3,Il2rb,Ccr5"
    labSize <- 12    
...
print("template_SuppFIGURE_2F_aPD1_vs_PBS.R #########################################################################")
...
var_SuppFIGURE_2F_aPD1_vs_PBS<-SuppFIGURE_2F_aPD1_vs_PBS(var_DEGMarkers_Reclustered_CD8CD3)
saveRDS(var_SuppFIGURE_2F_aPD1_vs_PBS, paste0(rds_output,"/var_SuppFIGURE_2F_aPD1_vs_PBS.rds"))
