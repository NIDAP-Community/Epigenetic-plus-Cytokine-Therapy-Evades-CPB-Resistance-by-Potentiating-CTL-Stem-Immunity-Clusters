# Filter Low Quality Cells [CCBR] [scRNA-seq] (8f4529f6-de08-4aad-89fd-41df365a749f): v67
QCFilteredSO <- function(ProcessedInputSO) {
...
mad.complexity.low = 5
topNgenes.high = NA
topNgenes.low = NA
mad.topNgenes.high = 5
mad.topNgenes.low = 5
n.topgnes=20
do.doublets.fitler=TRUE
filter.vdj.genes=TRUE
...
# Processing input variable: var_ProcessedInputSO
var_ProcessedInputSO<-readRDS(paste0(rds_output,"/var_ProcessedInputSO.rds"))

if (!('Seurat' %in% class(var_ProcessedInputSO))) { if (!(class(var_ProcessedInputSO) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_ProcessedInputSO <- as.data.frame(var_ProcessedInputSO)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_QCFilteredSO<-QCFilteredSO(var_ProcessedInputSO)
saveRDS(var_QCFilteredSO, paste0(rds_output,"/var_QCFilteredSO.rds"))
