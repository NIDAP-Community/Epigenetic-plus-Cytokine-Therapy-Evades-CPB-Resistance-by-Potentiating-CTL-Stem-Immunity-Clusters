...
SuppFIGURE_5E <- function(DEG_CD79bposCD3neg_Bcells) {
    # image: png
    ...
    #Basic Parameters:
    df.orig <- DEG_CD79bposCD3neg_Bcells
    label.col <- "Gene"
    sig.col <- c("C_T_ENT+N803+aPD1_vs_T_PBS_pval")
    pCutoff  = 0.05
    lfc.col <- c("C_T_ENT+N803+aPD1_vs_T_PBS_logFC")
    FCcutoff = 0.26303
    ...
    additional_labels <- "H2-Eb1,H2-Aa,Cd19,Cd79a,H2-Ab1,Ighd,Cd36,Cd86,H2-DMb2,Il2rg,Cd38,Ly6a,Cxcr4,H2-DMb1,Mzb1,Bach2,Ccr7,Cd37,Cd274,Pax5,Ebf1,Pou2af1"
    labSize <- 8    

    #Title and Axis labels Parameters
    change_sig_name <- "p-value"
    change_lfc_name <- "log2FC"
    title <- "ENP vs PBS_ B cells"
    #subtitle <- ""
...
print("template_SuppFIGURE_5E.R #########################################################################")
...
# Processing input variable: var_DEG_CD79bposCD3neg_Bcells
var_DEG_CD79bposCD3neg_Bcells<-readRDS(paste0(rds_output,"/var_DEG_CD79bposCD3neg_Bcells.rds"))

if (!('Seurat' %in% class(var_DEG_CD79bposCD3neg_Bcells))) { if (!(class(var_DEG_CD79bposCD3neg_Bcells) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_DEG_CD79bposCD3neg_Bcells <- as.data.frame(var_DEG_CD79bposCD3neg_Bcells)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_SuppFIGURE_5E<-SuppFIGURE_5E(var_DEG_CD79bposCD3neg_Bcells)
saveRDS(var_SuppFIGURE_5E, paste0(rds_output,"/var_SuppFIGURE_5E.rds"))
