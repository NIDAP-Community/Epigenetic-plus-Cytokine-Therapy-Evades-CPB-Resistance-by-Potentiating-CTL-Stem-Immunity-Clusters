...
DEGMarkers_Cd19posCd3neg <- function(ReclusterSO_Cd19posCd3neg,SampleNames_Cd19posCd3neg,RMDT_Cd19posCd3neg) {
    ...
    seurat_object <- ReclusterSO_Cd19posCd3neg
    metadata_table <- RMDT_Cd19posCd3neg
...
# Contrasts parameters
    ...
    contrasts <- c("ENT-PBS","N803-PBS","aPD1-PBS","ENT+aPD1-PBS","ENT+N803-PBS","N803+aPD1-PBS","ENT+N803+aPD1-PBS","ENT+N803+aPD1-ENT+aPD1","ENT+N803+aPD1-ENT+N803","ENT+N803+aPD1-N803+aPD1")
...
print("template_DEGMarkers_Cd19posCd3neg.R #########################################################################")
...
# Processing input variable: var_ReclusterSO_Cd19posCd3neg
var_ReclusterSO_Cd19posCd3neg<-readRDS(paste0(rds_output,"/var_ReclusterSO_Cd19posCd3neg.rds"))

if (!('Seurat' %in% class(var_ReclusterSO_Cd19posCd3neg))) { if (!(class(var_ReclusterSO_Cd19posCd3neg) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_ReclusterSO_Cd19posCd3neg <- as.data.frame(var_ReclusterSO_Cd19posCd3neg)}}
#############################


# Processing input variable: var_SampleNames_Cd19posCd3neg
var_SampleNames_Cd19posCd3neg<-readRDS(paste0(rds_output,"/var_SampleNames_Cd19posCd3neg.rds"))

if (!('Seurat' %in% class(var_SampleNames_Cd19posCd3neg))) { if (!(class(var_SampleNames_Cd19posCd3neg) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_SampleNames_Cd19posCd3neg <- as.data.frame(var_SampleNames_Cd19posCd3neg)}}
#############################


# Processing input variable: var_RMDT_Cd19posCd3neg
var_RMDT_Cd19posCd3neg<-readRDS(paste0(rds_output,"/var_RMDT_Cd19posCd3neg.rds"))

if (!('Seurat' %in% class(var_RMDT_Cd19posCd3neg))) { if (!(class(var_RMDT_Cd19posCd3neg) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_RMDT_Cd19posCd3neg <- as.data.frame(var_RMDT_Cd19posCd3neg)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_DEGMarkers_Cd19posCd3neg<-DEGMarkers_Cd19posCd3neg(var_ReclusterSO_Cd19posCd3neg,var_SampleNames_Cd19posCd3neg,var_RMDT_Cd19posCd3neg)
saveRDS(var_DEGMarkers_Cd19posCd3neg, paste0(rds_output,"/var_DEGMarkers_Cd19posCd3neg.rds"))
