...
DP_Bcells_FilteredSO <- function(DP_Bcells, DP_Bcells_MDT, SampleNames_CellTypes) {
...
library(plotly)
    ...
    seurat_object <- DP_Bcells
    
    # Category parameters    
    category_to_filter <- "DPbcells"
    seed <- 10
...
print("template_DP_Bcells_FilteredSO.R #########################################################################")
...
# Processing input variable: var_DP_Bcells
var_DP_Bcells<-readRDS(paste0(rds_output,"/var_DP_Bcells.rds"))

if (!('Seurat' %in% class(var_DP_Bcells))) { if (!(class(var_DP_Bcells) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_DP_Bcells <- as.data.frame(var_DP_Bcells)}}
#############################


# Processing input variable: var_DP_Bcells_MDT
var_DP_Bcells_MDT<-readRDS(paste0(rds_output,"/var_DP_Bcells_MDT.rds"))

if (!('Seurat' %in% class(var_DP_Bcells_MDT))) { if (!(class(var_DP_Bcells_MDT) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_DP_Bcells_MDT <- as.data.frame(var_DP_Bcells_MDT)}}
#############################
...
var_DP_Bcells_FilteredSO<-DP_Bcells_FilteredSO(var_DP_Bcells,var_DP_Bcells_MDT,var_SampleNames_CellTypes)
saveRDS(var_DP_Bcells_FilteredSO, paste0(rds_output,"/var_DP_Bcells_FilteredSO.rds"))
