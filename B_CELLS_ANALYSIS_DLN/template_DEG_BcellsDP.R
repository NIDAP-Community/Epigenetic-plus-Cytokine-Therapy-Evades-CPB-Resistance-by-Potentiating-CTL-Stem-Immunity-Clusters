...
DEG_BcellsDP <- function(ReclusterSO1,RSO_SN_BcellsDP,RSO_MDT_BcellsDP) {
    ...
    seurat_object <- ReclusterSO1
    metadata_table <- RSO_MDT_BcellsDP
...
# Processing input variable: var_ReclusterSO1
var_ReclusterSO1<-readRDS(paste0(rds_output,"/var_ReclusterSO1.rds"))

if (!('Seurat' %in% class(var_ReclusterSO1))) { if (!(class(var_ReclusterSO1) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_ReclusterSO1 <- as.data.frame(var_ReclusterSO1)}}
#############################


# Processing input variable: var_RSO_SN_BcellsDP
var_RSO_SN_BcellsDP<-readRDS(paste0(rds_output,"/var_RSO_SN_BcellsDP.rds"))

if (!('Seurat' %in% class(var_RSO_SN_BcellsDP))) { if (!(class(var_RSO_SN_BcellsDP) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_RSO_SN_BcellsDP <- as.data.frame(var_RSO_SN_BcellsDP)}}
#############################


# Processing input variable: var_RSO_MDT_BcellsDP
var_RSO_MDT_BcellsDP<-readRDS(paste0(rds_output,"/var_RSO_MDT_BcellsDP.rds"))

if (!('Seurat' %in% class(var_RSO_MDT_BcellsDP))) { if (!(class(var_RSO_MDT_BcellsDP) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_RSO_MDT_BcellsDP <- as.data.frame(var_RSO_MDT_BcellsDP)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_DEG_BcellsDP<-DEG_BcellsDP(var_ReclusterSO1,var_RSO_SN_BcellsDP,var_RSO_MDT_BcellsDP)
saveRDS(var_DEG_BcellsDP, paste0(rds_output,"/var_DEG_BcellsDP.rds"))
