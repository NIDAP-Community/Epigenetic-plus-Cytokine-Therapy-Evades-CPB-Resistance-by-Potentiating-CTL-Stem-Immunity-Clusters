...
SuppFIGURE_5D <- function(DEG_BcellsDP) {
    # image: png
    ...
    no_genes_to_label <- 100
    use_only_addition_labels <- TRUE
    additional_labels <- "Ly6a,H2-D1,H2-K1,Ly6c2,Ly6c1,Ly6e,H2-Q6,Ly6d,H2-T22,Tnf,Id3,H2-Q4"
    labSize <- 10    
...
print("template_SuppFIGURE_5D.R #########################################################################")
...
# Processing input variable: var_DEG_BcellsDP
var_DEG_BcellsDP<-readRDS(paste0(rds_output,"/var_DEG_BcellsDP.rds"))

if (!('Seurat' %in% class(var_DEG_BcellsDP))) { if (!(class(var_DEG_BcellsDP) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_DEG_BcellsDP <- as.data.frame(var_DEG_BcellsDP)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_SuppFIGURE_5D<-SuppFIGURE_5D(var_DEG_BcellsDP)
saveRDS(var_SuppFIGURE_5D, paste0(rds_output,"/var_SuppFIGURE_5D.rds"))
