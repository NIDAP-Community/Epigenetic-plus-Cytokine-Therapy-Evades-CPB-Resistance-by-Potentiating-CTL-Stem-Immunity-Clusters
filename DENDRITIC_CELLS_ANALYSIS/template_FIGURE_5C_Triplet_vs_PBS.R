...
FIGURE_5C_Triplet_vs_PBS <- function(DEGmarkers_Clec9aItgaeDP) {
    ...
    #Primary Inputs:
    deg_table <- DEGmarkers_Clec9aItgaeDP

    #Basic Parameters:
    gene_name_column <- "Gene"
    species = "Mouse"
    collections_to_include <- c("KEGG","GO","H")
    ...
    significance_column <- c("C_ENT+N803+aPD1_vs_PBS_pval")[1]
    significance_threshold <- 0.05
    fold_change_column <- c("C_ENT+N803+aPD1_vs_PBS_logFC")[1]
    fold_change_threshold <- 1.2
    minimum_number_of_DEG_genes <- 50
    
    #Visual Parameters:
    number_of_pathways_to_plot <- 21 
    plot_top_pathways_up <- FALSE
    pathways_to_use_up <- c("immune response","immune system process","MHC protein complex","response to stimulus","MHC class II protein complex","cell activation","leukocyte activation involved in immune response","peptide antigen binding","Antigen processing and presentation","regulation of tumor necrosis factor superfamily cytokine production")
    plot_top_pathways_down <- TRUE
    pathways_to_use_down <- c() 
    pathway_axis_label_font_size <- 12
    pathway_axis_label_max_length <- 50

    #For Bubble Plot:
    sort_bubble_plot_by <- "percent gene hits per pathway"
    plot_bubble_size <- "fdr corrected pval"  
    plot_bubble_color <- "Fisher's Exact pval"
    bubble_colors <- "blue to red"
...
print("template_FIGURE_5C_Triplet_vs_PBS.R #########################################################################")
...
var_FIGURE_5C_Triplet_vs_PBS<-FIGURE_5C_Triplet_vs_PBS(var_DEGmarkers_Clec9aItgaeDP)
saveRDS(var_FIGURE_5C_Triplet_vs_PBS, paste0(rds_output,"/var_FIGURE_5C_Triplet_vs_PBS.rds"))
