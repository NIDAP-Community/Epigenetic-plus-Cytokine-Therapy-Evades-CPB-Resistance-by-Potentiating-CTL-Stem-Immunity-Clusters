# Combine & Normalize [CCBR] [scRNA-seq] (7f919d9a-be13-43dd-967f-cbbc8c1cf813): v244
CombNormSO <- function(QCFilteredSO) {
...
# Processing input variable: var_QCFilteredSO
var_QCFilteredSO<-readRDS(paste0(rds_output,"/var_QCFilteredSO.rds"))

if (!('Seurat' %in% class(var_QCFilteredSO))) { if (!(class(var_QCFilteredSO) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_QCFilteredSO <- as.data.frame(var_QCFilteredSO)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_CombNormSO<-CombNormSO(var_QCFilteredSO)
saveRDS(var_CombNormSO, paste0(rds_output,"/var_CombNormSO.rds"))
