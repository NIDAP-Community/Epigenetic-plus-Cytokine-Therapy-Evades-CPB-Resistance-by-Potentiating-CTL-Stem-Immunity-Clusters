...
FIGURE_5Bpart2 <- function(FIGURE_5D_ENTN803aPD1_vs_PBS, GSEA_Filtered, DEGmarkers_Clec9aItgaeDP, Reclusteredmetadata_Clec9aItgaeDP, msigdb_v6_2_with_orthologs) {
...
## PARAMETERS
    ...
    plot_limit = "10"
    topline = 'coordinate'
    pathways = c("GO_REGULATION_OF_ALPHA_BETA_T_CELL_ACTIVATION")
    geneid = "Gene"
    geneid_gex = "Gene"
    contrasts = c()
    species = "Mouse"

    genescore_df <- FIGURE_5D_ENTN803aPD1_vs_PBS
    gex_df <- DEGmarkers_Clec9aItgaeDP
    metadata_df <- Reclusteredmetadata_Clec9aItgaeDP
...
if (grepl("LE", which_plot)) {
    ...
    samples_to_include = setdiff(c("C_ENT+N803+aPD1_vs_PBS_pval"), geneid)
...
for ( i in 1:length(gsea_list) ) {
    ...
    if (which_plot == "ES+RNK+LE") {
        ...
        metadata = prep.metadata(meta=metadata_df, gsea=gsea, samples=colnames(gex_le), Sample="My_Sample_Names", Group="Clec9aItgaeDP", transformation=gex_transformation, dropREF=drop_ref, reference=reference_phenotype, own_palette=c(), label_palette='Accent', filterSamples=filter_samples)
...
png(filename="FIGURE_5Bpart2.png", width=(pageWidth+5)*300, height=(pageHeight+5)*300, units="px", pointsize=4, bg="white", res=300, type="cairo") 
print(preview)
...
print("template_FIGURE_5Bpart2.R #########################################################################")
...
# Processing input variable: var_FIGURE_5D_ENTN803aPD1_vs_PBS
var_FIGURE_5D_ENTN803aPD1_vs_PBS<-readRDS(paste0(rds_output,"/var_FIGURE_5D_ENTN803aPD1_vs_PBS.rds"))

if (!('Seurat' %in% class(var_FIGURE_5D_ENTN803aPD1_vs_PBS))) { if (!(class(var_FIGURE_5D_ENTN803aPD1_vs_PBS) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_FIGURE_5D_ENTN803aPD1_vs_PBS <- as.data.frame(var_FIGURE_5D_ENTN803aPD1_vs_PBS)}}
#############################


# Processing input variable: var_GSEA_Filtered
var_GSEA_Filtered<-readRDS(paste0(rds_output,"/var_GSEA_Filtered.rds"))

if (!('Seurat' %in% class(var_GSEA_Filtered))) { if (!(class(var_GSEA_Filtered) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_GSEA_Filtered <- as.data.frame(var_GSEA_Filtered)}}
#############################


# Processing input variable: var_DEGmarkers_Clec9aItgaeDP
var_DEGmarkers_Clec9aItgaeDP<-readRDS(paste0(rds_output,"/var_DEGmarkers_Clec9aItgaeDP.rds"))

if (!('Seurat' %in% class(var_DEGmarkers_Clec9aItgaeDP))) { if (!(class(var_DEGmarkers_Clec9aItgaeDP) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_DEGmarkers_Clec9aItgaeDP <- as.data.frame(var_DEGmarkers_Clec9aItgaeDP)}}
#############################
...
var_FIGURE_5Bpart2<-FIGURE_5Bpart2(var_FIGURE_5D_ENTN803aPD1_vs_PBS,var_GSEA_Filtered,var_DEGmarkers_Clec9aItgaeDP,var_Reclusteredmetadata_Clec9aItgaeDP,var_msigdb_v6_2_with_orthologs)
saveRDS(var_FIGURE_5Bpart2, paste0(rds_output,"/var_FIGURE_5Bpart2.rds"))
