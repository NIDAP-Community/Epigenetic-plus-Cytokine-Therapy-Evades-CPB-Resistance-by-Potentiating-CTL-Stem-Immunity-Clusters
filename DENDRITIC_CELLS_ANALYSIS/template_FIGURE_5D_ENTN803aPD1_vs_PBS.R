...
FIGURE_5D_ENTN803aPD1_vs_PBS <- function(DEGmarkers_Clec9aItgaeDP) {
    # image: png
    ...
    #Basic Parameters:
    df.orig <- DEGmarkers_Clec9aItgaeDP
    label.col <- "Gene"
    sig.col <- c("C_ENT+N803+aPD1_vs_PBS_pval")
    pCutoff  = 0.05
    lfc.col <- c("C_ENT+N803+aPD1_vs_PBS_logFC")
    FCcutoff = 0.26303
    ...
    no_genes_to_label <- 0
    use_only_addition_labels <- TRUE
    additional_labels <- "H2-Aa,H2-Ab1,H2-Oa,H2-Eb1,Cd274,H2-Ob,H2-Eb2,Cd53,Pdcd1lg2,H2-D1,Ccr2,H2-Q10,Ifngr1,Batf3,Jak2,Cd86,Cd81,Clec9a"
    labSize <- 8    

    #Title and Axis labels Parameters
    change_sig_name <- "p-value"
    change_lfc_name <- "log2FC"
    title <- "ENTN803aPD1 vs PBS"
    #subtitle <- ""
...
print("template_FIGURE_5D_ENTN803aPD1_vs_PBS.R #########################################################################")
...
var_FIGURE_5D_ENTN803aPD1_vs_PBS<-FIGURE_5D_ENTN803aPD1_vs_PBS(var_DEGmarkers_Clec9aItgaeDP)
saveRDS(var_FIGURE_5D_ENTN803aPD1_vs_PBS, paste0(rds_output,"/var_FIGURE_5D_ENTN803aPD1_vs_PBS.rds"))
