...
ExcludeNK_CD8Tcells <- function(FSO_CD3CD8_Tcells,SampleNames_CellTypes) {  # produces 2 color channels and the overlay
    ...
    marker1 <- "Itga2"
    marker_1_threshold <- 0.018
    marker_1_type <- "SCT"
    marker2 <- "Cd4"
    marker_2_threshold <- 0.025
    marker_2_type <- "SCT"
    
    #Filter Parameters:
    filter_data <- TRUE
    parameter_name <- "Cd49b_Cd4_neg"
...
print("template_ExcludeNK_CD8Tcells.R #########################################################################")
...
var_ExcludeNK_CD8Tcells<-ExcludeNK_CD8Tcells(var_FSO_CD3CD8_Tcells,var_SampleNames_CellTypes)
saveRDS(var_ExcludeNK_CD8Tcells, paste0(rds_output,"/var_ExcludeNK_CD8Tcells.rds"))
