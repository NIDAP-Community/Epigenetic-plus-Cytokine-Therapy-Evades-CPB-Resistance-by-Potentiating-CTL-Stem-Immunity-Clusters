# Process Input Data [CCBR] [scRNA-seq] (4b79d66e-50f0-42da-8c47-dd565dab2257): v61
ProcessedInputSO <- function(Dataset_CM59_DLNandTUMOR,My_MetadataToAdd) {
...
input = Dataset_CM59_DLNandTUMOR
sample.metadata.table= My_MetadataToAdd
use.metadata.table= TRUE
...
input.dat <- input[grepl('\\.h5$', input)]; input.tcr <- input[grepl('\\.csv$', input)]; input=c(input.dat,input.tcr);
}
...
print("template_ProcessedInputSO.R #########################################################################")
...
# Processing input variable: var_Dataset_CM59_DLNandTUMOR
var_Dataset_CM59_DLNandTUMOR<-readRDS(paste0(rds_output,"/var_Dataset_CM59_DLNandTUMOR.rds"))

if (!('Seurat' %in% class(var_Dataset_CM59_DLNandTUMOR))) { if (!(class(var_Dataset_CM59_DLNandTUMOR) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_Dataset_CM59_DLNandTUMOR <- as.data.frame(var_Dataset_CM59_DLNandTUMOR)}}
#############################


# Processing input variable: var_My_MetadataToAdd
var_My_MetadataToAdd<-readRDS(paste0(rds_output,"/var_My_MetadataToAdd.rds"))

if (!('Seurat' %in% class(var_My_MetadataToAdd))) { if (!(class(var_My_MetadataToAdd) %in% c('data.frame', 'RFilePath', 'character', 'list'))) {var_My_MetadataToAdd <- as.data.frame(var_My_MetadataToAdd)}}
#############################

# Saving Output Dataset
invisible(graphics.off())
var_ProcessedInputSO<-ProcessedInputSO(var_Dataset_CM59_DLNandTUMOR,var_My_MetadataToAdd)
saveRDS(var_ProcessedInputSO, paste0(rds_output,"/var_ProcessedInputSO.rds"))
